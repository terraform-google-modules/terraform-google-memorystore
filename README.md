...
| maintenance\_policy | The maintenance policy for an instance. | <pre>object({<br>    day = string<br>    start_time = {<br>      hours   = number<br>      minutes = number<br>      seconds = number<br>      nanos   = number<br>    }<br>  })</pre> | <pre>{<br>  "day": null,<br>  "start_time": {<br>    "hours": null,<br>    "minutes": null,<br>    "nanos": null,<br>    "seconds": null<br>  }<br>}</pre> | no |
